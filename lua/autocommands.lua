...
-- Quickfix window: <C-n>/<C-p> jump entry-by-entry. j/k stay as default line nav.
-- Buffer-local; doesn't shadow <C-n>/<C-p> elsewhere.
vim.api.nvim_create_autocmd('FileType', {
  pattern = 'qf',
  callback = function(args)
    local opts = { buffer = args.buf, silent = true }
    vim.keymap.set('n', '<C-n>', '<cmd>cnext<CR><cmd>copen<CR>', opts)
    vim.keymap.set('n', '<C-p>', '<cmd>cprev<CR><cmd>copen<CR>', opts)
  end,
})
